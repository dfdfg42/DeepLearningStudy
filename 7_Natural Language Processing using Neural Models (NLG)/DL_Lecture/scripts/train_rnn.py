...
from DL_Lecture.models.nmt_rnn import EncoderLSTM_Att, DecoderLSTM_Att
from DL_Lecture.utils.text_prepro import buildVocab, load_nmt_pair_data, text_to_indices
from DL_Lecture.utils.data_prepro import TranslateDataset, collate
...
def main():
    ...
    if params['task'] == "EN2DE":
        ...
        train_source_text, train_target_text = load_nmt_pair_data(data_params['train_file']) # 탭으로 구분해서 트레인 소스랑 테스트 소스 구분해서 넣어주기
    ...
    # Create dataloaders
    train_dataloader = DataLoader(TranslateDataset(train_x, train_y, device), batch_size=params['batch_size'],
                                  shuffle=True, collate_fn=collate, drop_last=True)
    dev_dataloader = DataLoader(TranslateDataset(dev_x, dev_y, device), batch_size=params['batch_size'],
                                collate_fn=collate, drop_last=True)

    # 학습 모델 생성
    # Initialize your models
    encoder = EncoderLSTM_Att(vocab_size_source, params['embedding_dim'], params['model_params_rnn']['hidden_size'],
                              initW).to(device)
    decoder = DecoderLSTM_Att(params['embedding_dim'], params['model_params_rnn']['hidden_size'], vocab_size_target).to(
        device)
    ...
    for epoch in range(params['max_epochs']):
        ...
        for i, training_pair in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            ...
            encoder_output, encoder_hidden, encoder_cell = encoder(input_tensor,
                                                                   input_length)  # encoder에 sentence, 길이, initial state 먹이기
            ...
            use_teacher_forcing = True if random.random() < params['model_params_rnn'][
            ...
            writer.add_scalar(tag='Batch/Loss', scalar_value=loss.item() / params['batch_size'],
            ...
            with torch.no_grad():
                ...
                for di in range(max_output_length):
                    ...
                    for q in range(params['batch_size']):
                        inference_words[q].append(
                            id2word_target[topi[q].item()])  # 추론 결과 확인을 위해 index를 word로 넣어서 list에 보관
