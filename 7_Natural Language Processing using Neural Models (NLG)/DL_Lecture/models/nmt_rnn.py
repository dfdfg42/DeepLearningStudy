...
class DecoderLSTM_Att(nn.Module):
    ...
    def forward(self, enc_output, input, hidden, cell):
        ...
        att_score = torch.bmm(enc_output, hidden.permute((1, 2, 0)))  # 내적한 결과 -> 어텐션 얼마나 닯았는지
        # bmm : batch-wise matrix multiplication
        # [batch_size, length, 1]

        # X1: attention score에 softmax를 적용하여 attention distribution 생성
        att_dist = torch.softmax(att_score, dim=1)  # [batch_size, length, 1]
        ...
        # X2: concat된 hidden을 원래 크기로 줄이기 위해 Linear layer 사용
        hidden = self.att_combine(hidden)  # [1, batch_size, hidden_size]

        # hidden : [1, batch_size, hidden_size]
        # X3: hidden을 output vocabulary size로 projection
        output = self.softmax_final(self.out(hidden[0]))  # LogSoftmax 적용하여 output 클래스로 단어 projection
